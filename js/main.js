...
function createBloom(counter, iParam) {
    ...
    // Use last color palette
    var randomPaletteIndex = NUM_PALETTES - 1;
    ...
    for (var j = numRings - 1; j >= 1; j--) {
        console.log(j);
        circle_array.push(new g_circle_t(counter, 1, j, 0, x, y, init_r, init_r + init_max_radius + 5 * j, rgbColors[j].r, rgbColors[j].g, rgbColors[j].b, max_radius * 2, 0));
    }
}
...
function g_circle_t(counter, opacity, ring, wait, x, y, min_r, max_r, red, green, blue, max_time, invisflag){
...
function update_circle_state(g_circle) {
    if (g_circle.wait <= 0) {
        if (g_circle.is_fading_away) {
                var blur = Math.max((NUM_RINGS_PER_BLOOM - g_circle.ring), 3) / Math.pow(Math.E, g_circle.time / g_circle.max_time);
